...
public class TSP {
	...
	/**
	 * Searches for a tour in <code>graph</code> lower than <code>bound</code>
	 * and tries to prove its optimality in less than <code>timeLimit</code>
	 * milliseconds.
	 * 
	 * @param timeLimit
	 *            a limit of CPU time in milliseconds
	 * @param bound
	 *            an upper bound on the total cost of the tour
	 * @return NO_SOLUTION_FOUND, if no tour lower than <code>bound</code> has
	 *         been found within <code>timeLimit</code> milliseconds,<br>
	 *         SOLUTION_FOUND, if a tour lower than <code>bound</code> has been
	 *         found, but its optimality has not been proven within
	 *         <code>timeLimit</code> milliseconds,<br>
	 *         OPTIMAL_SOLUTION_FOUND, if a tour lower than <code>bound</code>
	 *         has been found, and its optimality has been proven,<br>
	 *         or INCONSISTENT, if there does not exist a tour lower than
	 *         <code>bound</code>.
	 */
	public SolutionState solve(int timeLimit, int bound) {
		...
		for (int i = 0; i < n; i++)
			xNext[i] = VariableFactory.enumerated("Next " + i,
					graph.getSucc(i), solver);
	...
	/**
	 * @return an array <code>next</code> such that <code>next[i]</code> gives
	 *         the vertex visited just after <code>i</code> in the last computed
	 *         solution
	 * @throws NullPointerException
	 *             If <code>this.getSolutionState()</code> not in
	 *             <code>{SOLUTION_FOUND, OPTIMAL_SOLUTION_FOUND}</code>
	 */
	public int[] getNext() throws NullPointerException {
		if ((state == SolutionState.NO_SOLUTION_FOUND)
				|| (state == SolutionState.INCONSISTENT))
	...
	/**
	 * @return NO_SOLUTION_FOUND if no solution has been found during the last
	 *         call to solve,<br>
	 *         SOLUTION_FOUND if a solution has been found during the last call
	 *         to solve but its optimality has not been proven,<br>
	 *         OPTIMAL_SOLUTION_FOUND if a solution has been found during the
	 *         last call to solve, and its optimality proven,<br>
	 *         INCONSISTENT if the last call to solve has proven that the
