...
public class ShortestPathGraph implements Graph {
	 ...
	 * 
	 * @param Map
	 *            <Integer, Map<Integer,Path>> pathMap the first key is the id
	 *            of the origin node the second key is the id of the destination
	 *            node
	 */
	 ...
	 * 
	 * @param Map
	 *            <Integer, Map<Integer,Path>> pathMap
	 */
	private void initializeCosts(Map<Integer, Map<Integer, Path>> pathMap) {
		...
		for (Map.Entry<Integer, Map<Integer, Path>> entry : pathMap.entrySet()) {
			...
			for (Map.Entry<Integer, Path> value : entry.getValue().entrySet()) {
				...
				// updating the cost map and the array of successors for each
				// key.
